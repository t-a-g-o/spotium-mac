...
BLOCKTHESPOT_VERSION="11.2.32.985"
...
FAILED_VERSION_FILE="/Users/${USER}/Spotium/failedversion.txt"
if [[ $(ver "${CLIENT_VERSION}") -lt $(ver "${BLOCKTHESPOT_VERSION}") ]]; then
  if [[ -f "/Users/${USER}/Spotium/bypass.spotium" ]]; then
    rm "/Users/${USER}/Spotium/bypass.spotium"
    echo "By pass exists"
  else
    if [ ! -f "$FAILED_VERSION_FILE" ]; then
        touch "$FAILED_VERSION_FILE"
    fi
    echo "failed"
    echo "${BLOCKTHESPOT_VERSION}\n${CLIENT_VERSION}" > "$FAILED_VERSION_FILE"
    exit
  fi
fi
